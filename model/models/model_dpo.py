...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def __init__(self, pretrained_model, **kwargs):
        ...
        # =========================================================================================
        module_kwargs, _, _ = self._split_kwargs(kwargs)
        self.accelerator = Accelerator()
        index_path = module_kwargs.get("index_path", None)
        if index_path is not None:
            Settings.embed_model = HuggingFaceEmbedding(model_name="BAAI/bge-base-en-v1.5")
            storage_context = StorageContext.from_defaults(persist_dir=index_path)
            self.retriever = load_index_from_storage(storage_context).as_retriever()
        else:
            self.retriever = None
    ...
    def forward(
        ...
        **kwargs,
        ...
        # TODO maybe add return_dict=False to the forward call
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        input_ids = torch.cat(
            [chosen_inputs.input_ids, rejected_inputs.input_ids], dim=0
        )
        attention_mask = torch.cat(
            [chosen_inputs.attention_mask, rejected_inputs.attention_mask], dim=0
        )
        ...
        gen_probs = (
            torch.gather(probs, 2, input_ids[:, :, None]).squeeze(-1).mean(dim=1)
        )
        chosen_logps = gen_probs[: len(batch["chosen"])]
        rejected_logps = gen_probs[len(batch["chosen"]) :]
    ...
    def prediction_step_reward(
        ...
        beta = 0.1
        # losses = -F.logsigmoid(beta * logits)  # also known as -\log \sigma(h_{\pi_\theta}^{y_w,y_l})
        # # losses = -F.logsigmoid(beta * logits) * (1 - label_smoothing) - F.logsigmoid(-beta * logits) * label_smoothing

        # print('policy_chosen_logps: ', policy_chosen_logps.shape)

        chosen_rewards = beta * (policy_chosen_logps - reference_chosen_logps).detach()
        rejected_rewards = (
            beta * (policy_rejected_logps - reference_rejected_logps).detach()
        )
        output_dict["chosen_rewards"] = chosen_rewards  # .mean(dim=1)
        output_dict["rejected_rewards"] = rejected_rewards  # .mean(dim=1)
        return output_dict
    ...
    def prediction_step_mcqa(self, batch, tokenizer):
        ...
        for question in tqdm(batch["question"]):
            formatted_question = "<|user|>\n"
            if self.retriever is not None:
                context = self._get_context(question)
                formatted_question += f"Context: {context}\n"
            formatted_question += "You will answer an MCQ question. You will be given the question, followed by the four possible answers (A , B , C , D )."
            formatted_question += "You will reply only with one character corresponding to the correct answer.\n"
            formatted_question += f"{question}\n"
            formatted_question += "<|end|>\n"
            formatted_question += "<|assistant|>\n"
            ...
            id_to_token = {319: "A", 350: "B", 315: "C", 360: "D"}
            token_probs = {}
            for token_id in token_ids:
                token_probs[id_to_token[token_id]] = probabilities[token_id].item()
            final_answer = max(token_probs, key=token_probs.get)
            output_dict["preds"].append(final_answer)
            print(final_answer)
